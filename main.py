...
for i in top_free_apps:
    ...
    obj = App(app_img=img[0]["data-src"], app_name=name[0].text, app_pub=pub[0].text[:20],
...
<div id = "d">Top Free Apps</div><br>""")
        ...
        for apps in applications:
            if c == 0:
                ...
                self.response.write(
                    '<img id = "app1_img" src = "%s" width = "120px" height="120px"><br><br>' % apps.app_img)
                self.response.write('%s<br>' % apps.app_name)
                # 			self.response.write('%s<br>' %app.app_pub)
                self.response.write('</p>')
            elif c == 1:
                self.response.write('<p id = "app2">')
                self.response.write(
                    '<img id = "app2_img" src = "%s" width = "120px" height="120px"><br><br>' % apps.app_img)
                self.response.write('%s<br>' % apps.app_name)
                # 			self.response.write('%s<br>' %app.app_pub)
                self.response.write('</p>')
            elif c == 2:
                ...
                self.response.write(
                    '<img id = "app3_img" src = "%s" width = "120px" height="120px"><br><br>' % apps.app_img)
                self.response.write('%s<br>' % apps.app_name)
                # 			self.response.write('%s<br>' %app.app_pub)
                self.response.write('</p>')
            else:
